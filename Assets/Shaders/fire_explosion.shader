...
uniform float shape_scale = 4.0;
uniform float shape_y_scale = 0.6;
...
void fragment(){
	...
	float shape = texture( shape_tex, vec2(UV.x*shape_scale, UV.y *shape_scale * shape_y_scale + TIME * scroll_speed)).r;
	float alpha = texture( shape_tex, vec2(UV.x, UV.y * shape_y_scale + TIME * scroll_speed)).a;
