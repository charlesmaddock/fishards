...
void fragment(){
	float circle = 1.0 - distance(UV,vec2(0.5))*2.0;
	float noise = ( fbm(UV *scaling + TIME, 6) + fbm(UV *scaling/2.0 + TIME, 6) ) /2.0;
	vec3 color = mix(color1.rgb, color2.rgb, noise) * circle;
	ALBEDO = color;
	ALPHA = circle;
}
